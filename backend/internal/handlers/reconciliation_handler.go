...
func UploadReconciliationFiles(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"match_rate":          fmt.Sprintf("%.1f%%", report.ActionableMatchRate()),
		"match_rate_overall":  fmt.Sprintf("%.1f%%", report.MatchRate()),
		"bank_payments":       report.TotalPayments(),
		"bank_out_of_range":   len(report.BankOutOfRange),
		"bank_discrepancies":  len(report.BankTrueDiscrepancies),
		"foundation_out_of_range": len(report.FoundationOutOfRange),
		"foundation_discrepancies": len(report.FoundationTrueDiscrepancies),
